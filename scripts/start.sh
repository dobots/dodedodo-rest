...
echo "Make sure you did run ./devices.sh database"
...
# Just plug in the device to start too
sed -i "s|\"description\"|\"start\":$device, \"description\"|" module.txt

content=$(cat module.txt)
#echo $content

curl -b cookie $DODEDODO_SERVER:$DODEDODO_PORT/modules/start -H "Content-Type: application/json" --data "$content"
